...
The purpose of this package is to provide you with easy syntax for making an anonymous
function. 
...
## Similar notation
Family of `purrr::map` such as `map_if()` or `map_dbl()` can accept formula 
notation with `..1` or `.x` which is converted to a closure by `rlang::as_closure()`.
See comparison of usage below.
...
The "De Bruijn index" is suitable for a notation of lambda culculas, 
but double-tilda is degined to cooporate with normal use of R's functions,
not curried-function.
See https://en.wikipedia.org/wiki/De_Bruijn_index for "De Bruijn index".


### f.
A function name `f()` is used in many situation, so I avoid using it and `f.()` 
is adopted.
